...
class MathEngine(object):
    ...
    def solve(self,style,compind=0.5,tristind=1,nervind=1,alc=0):
        self.alc = alc
        if alc == 0:
        ...
        ##            print soda_min_sorted
                ...
                for recipe in tabrecipes:
                    if alc >= int(recipe['alcool']):
                        score = math.floor(float(complexite)*compind - float(recipe['complex'])) + math.floor(float(tristesse)*tristind - float(recipe['tristesse'])) + math.floor(float(enervement)*nervind - float(recipe['nerf']))
                        if self.debug:
                ...
                qty = 5/(int(cock['qte1'])+int(cock['qte2'])+int(cock['qte3'])+int(cock['qte4'])+int(cock['qte5'])+int(cock['qte6']))
