...
def arggsort(data):
    ...
    ind_start = 0 # début de la plage (inclus) pour laquel les valeurs ordonnées sont identiques
    ind_stop = 1 # fin de la plage (exclus) pour laquel les valeurs ordonnées sont identiques
...
def PIANOCKTAIL(fname):
    ...
    # OUI c'est normal !!!
    tristesse = max(45*(1-tempo/150) + 40*np.floor(tonalite/13.), 0) + 15*(1-np.floor(metrique/3.))
...
def TONALITE(note_piece):
    ...
    for k in np.arange(size_pc):
        pcds[0, pc[k]] = pcds[0, pc[k]]+dur_acc[k]
...
def TEMPO(note_piece):
    ...
    # là ça vaut 0
    # OUI, il faut initialiser à 1 pour tenir compte de la note qu'on vient de classer
    Note_classees[0, 2] = 1  
    ...
    Poids_tempo[:, 5] = 1*Poids_tempo[:, 1] + 1*Poids_tempo[:, 2] + 2*Poids_tempo[:, 3] + 2*Poids_tempo[:, 4]
    ...
    indices = np.nonzero(Note_classees[:,0] > (Note_classees[Nb_att-1, 0]/12))[0]
    ...
    # OK, ma modif est bonne, comme ça marchait Tom n'a pas repéré ("wild programming")
    ...
    battuesap = Note_classees[(i1+1):(i1+2+np.fix(Nb_att/6)),0] - Note_classees[i1,0]
    ...
    for i in np.arange(Nb_battues):
        ...
        ind_max = min(i2+np.floor(liste_battues[i, 0]/tolerance)+1, Nb_att-2)
        ...
        while (distance < crit_dist) and ((ind_max) < Nb_att-1):
            ...
            if i2+ind_possible < Note_classees.shape[0]:
                temp_batt.append(abs(Note_classees[i2, 0] - Note_classees[i2+ind_possible, 0]))
            else:
                break
            ...
            liste_battues[i, 1] = Note_classees[i2+ind_possible, 0] - Note_classees[i1, 0]
            ...
            # OUI le commentaire est faux ind_possible est modifié à la dernière ligne de cette boucle
            i2 += ind_possible
            ind_max = min(i2+np.floor(liste_battues[i, 0]/tolerance)+1, Nb_att-2)
    ...
    # OK, comme l'algo apprend la batue, il peut s'adapter et tomber sur les mêmes valeurs
    ...
    # FJ !!! : ajout pour probleme fichier bug_4.pckt. Voir coment bien gérer ce cas
    if tempo < 0.:
        tempo = 1.
...
def METRIQUE(note_piece):
    ...
    # on utilise la fonction round par défaut de Python qui a le même comportement que dans scilab :
    ...
    ac = np.correlate(of-of.mean(),of-of.mean(),mode='full')[of.shape[0]-1:]
...
#    print(a3)
    ...
    for file_name in file_names :
        duree, tristesse, Enervement, Complexite, metrique, tonalite, tempo, cocktail = PIANOCKTAIL(file_name)
